...
class RumViewController: UIViewController, IndicatorInfoProvider, UITableViewDataSource, UITableViewDelegate{
    ...
    func updateData(){
        LoadingProxy.on()
        CocktailAPI.getCocktails(id: "4"){(result, error) in
            if error == nil{
                LoadingProxy.off()
    ...
    func tableView(_ tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        let nextView = storyboard.instantiateViewController(withIdentifier: "detail") as! RumDetailViewController
