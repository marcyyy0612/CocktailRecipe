...
class TequilaViewController: UIViewController, IndicatorInfoProvider, UITableViewDataSource, UITableViewDelegate{
    ...
    func updateData(){
        LoadingProxy.on()
        CocktailAPI.getCocktails(id: "3"){(result, error) in
            if error == nil{
                LoadingProxy.off()
    ...
    func tableView(_ tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        let nextView = storyboard.instantiateViewController(withIdentifier: "detail") as! TequilaDetailViewController
